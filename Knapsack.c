...
void selection_sort(double *arr, int *sorted_index, int num) {
...
void max_value(int *weights, int *values, int num, int capacity) {
    ...
    selection_sort(ratios, sorted_index, num);
...
void max_weight(int *weights, int *values, int num, int capacity) {
	...
	selection_sort(sorted_weights, sorted_index, num);
    ...
    for (int i = 0; i < num; i++) {
        ...
        printf("Item %d: Weight: %d, Value: %.2f\n", i + 1, sorted_weights[i], total_value);
        overall_value += total_value;
    }
    printf("Overall Value is: %.2f\n", overall_value);
}
